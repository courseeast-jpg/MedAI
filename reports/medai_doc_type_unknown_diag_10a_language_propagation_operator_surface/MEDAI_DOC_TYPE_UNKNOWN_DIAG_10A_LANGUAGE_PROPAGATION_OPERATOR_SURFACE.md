...
Both the DIAG-08A numeric-table operator badge and the new DIAG-10A language-propagation metadata display are now available behind separate default-off env vars. A future evaluation-only block (e.g. UNKNOWN-DIAG-11A) may begin to characterize the remaining deferred pools - the 8 latin medical-abbreviation records, the 1 table-header record, the 21 text-layer records, and the 17 fallback records - to decide whether any further default-off helper is justified. Cue expansion remains not recommended for any pool.
