"""CKA-SEC-07 — operator-facing restore CLI for an encrypted CKA backup.

Modes:
  --dry-run                              Synthetic round-trip in temp paths;
                                         no real DB touched.
  --backup <path> --target <path>        Real restore. Operator key prompted
                                         twice via getpass; never echoed,
                                         never stored, never logged.
  --overwrite                            Permit overwriting an existing
                                         target (default refuses).
  --test-mode                            (test-only) Read key from
                                         CKA_SEC04_TEST_KEY env var.

Hard rules:
- The encryption key is NEVER accepted on the command line.
- The encryption key is NEVER printed, written to disk, or logged.
- Wrong key on the restored DB raises — no silent fallback.
"""
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="cka_encrypted_store_restore",
        description="Restore an encrypted CKA backup to a target path.",
    )
    p.add_argument("--backup", default=None, help="Path to the backup file.")
    p.add_argument("--target", default=None, help="Path where the restored DB will be written.")
    p.add_argument("--dry-run", action="store_true",
                   help="Synthetic round-trip in temp paths; no real DB touched.")
    p.add_argument("--overwrite", action="store_true",
                   help="Permit overwriting an existing target.")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.dry_run:
        ...
        tgt = _new_temp_db("cka_sec07_dryrun_tgt_")
        try:
            ...
            _print_safe_summary("CKA-SEC-07 restore (dry-run)", result.safe_public_summary())
    ...
    if not args.backup or not args.target:
        print("ERROR: --backup and --target are required in non-dry-run mode.",
              file=sys.stderr)
        return 2
    ...
    _print_safe_summary("CKA-SEC-07 restore", result.safe_public_summary())
    key = ""    # noqa: F841
    del key
    return 0 if result.success else 1
