"""MEDAI-DOC-TYPE-UNKNOWN-DIAG-09A - Language Detector Metadata Propagation Spec.

Specification-only block.

Scope
-----
Targets the 11 records DIAG-04 routed to
``language_detector_metadata_propagation_audit``. The block:

    1. Re-derives the priority slice deterministically via the DIAG-02 ->
       DIAG-03 -> DIAG-04 chain (no corpus rerun, no source documents).
    2. Confirms every record in the slice matches the exact positive
       signal pattern below.
    3. Confirms no exclusion rule fires on the slice.
    4. Confirms 0 overlap with the 11 numeric-table safe-default records
       already handled by DIAG-06A/07A/08A.
    5. Emits a privacy-safe public spec of a FUTURE default-off
       propagation implementation: positive signal pattern, exclusion
       rules, proposed propagation behavior, acceptance criteria,
       validation requirements, and rollback boundaries.

This block does NOT implement propagation behavior. It does not write
classifier behavior, modify runtime decisions, or change any system
component.
...
* No language metadata propagation behavior changes in this block.
...
Progress estimate
-----------------
* Before this block: residual Unknown-reduction track ~76% done /
  ~24% remaining; whole MedAI project ~79% done / ~21% remaining.
* After this block:  residual Unknown-reduction track ~80% done /
  ~20% remaining; whole MedAI project ~80% done / ~20% remaining.
"""
...
OUTPUT_DIR = REPO_ROOT / "reports" / "medai_doc_type_unknown_diag_09a"

SOURCE_08A_COMMIT_SHORT = "f5b80ce"
...
_DIAG04_TARGET_LEVER = "language_detector_metadata_propagation_audit"
...
PROPOSED_FUTURE_PROPAGATION: dict[str, Any] = {
    "applies_to": (
        "records matching the exact positive signal pattern only"
    ),
    "default_action": (
        "propagate detector-side latin/likely-english context into safe "
        "metadata only; record the propagation as a derived field "
        "alongside the raw detector output, never replacing it"
    ),
    "scope_of_effect": (
        "metadata propagation only; raw detector output is untouched, "
        "classifier outcome is unchanged, no clinical interpretation, no "
        "value parsing"
    ),
...
class DiagnosticReport:
    ...
    total_propagation_pool_records: int
    ...
    propagation_implemented_in_this_block: bool
...
def select_propagation_pool(table: list[dict]) -> list[dict]:
    """Return the records DIAG-04 routed to language_detector_metadata_propagation_audit."""
...
def _positive_signal_predicate(key: str, expected: str, record: dict) -> bool:
    ...
    if key == "detector_output_not_propagated":
        # Synthetic: high-confidence Latin detector input that the family
        # classifier never propagated language for.
        return (
...
def _positive_signal_match_report(
    records: list[dict],
...
def _exclusion_rule_violates(rule: str, record: dict) -> bool:
    ...
    if rule == "exclude_table_heavy_numeric_safe_default_records_already_handled":
        ...
        # handled by the upstream pipeline and must be excluded here.
...
def build_diagnostic_from_report(source_payload: dict) -> DiagnosticReport:
    ...
    pool = select_propagation_pool(table)
    ...
    raw_signal_counts = {
        ...
        "language_script_detector_unknown_bucket_counts":
            _safe_count(pool, "language_script_detector_unknown_bucket"),
    ...
    deferred_subsets = {
        "numeric_table_safe_default_pool_already_handled":
            "11 records covered by DIAG-06A/07A/08A; excluded from this spec",
    ...
    if sig_holds and excl_clean and no_overlap and len(pool) > 0:
        rec_next = "future_block_named_unknown_diag_09a_implementation"
        rec_justification = (
            f"All {len(pool)} propagation-pool records match the exact "
            f"positive signal pattern, no exclusion rule fires, and there is "
            f"zero overlap with the numeric-table safe-default pool. A future "
            f"implementation block may prototype the metadata propagation "
            f"behavior inside the published acceptance criteria for that "
            f"future block. The implementation must remain review-bound, "
            f"default-off behind a separate env flag or operator setting, and "
            f"must not alter raw detector output, classifier behavior, OCR "
            f"routing, or any clinical interpretation."
        )
    else:
        ...
        rec_justification = (
            ...
            "numeric-table safe-default pool. Revise the spec before any "
            "implementation is considered. All records remain review-bound."
        )
    ...
    safety_privacy = {
        ...
        "no_overlap_with_numeric_table_safe_default_pool": no_overlap,
    }

    return DiagnosticReport(
        snapshot="MEDAI-DOC-TYPE-UNKNOWN-DIAG-09A",
        branch=_git_branch(),
        head_commit_short=_git_head()[:12],
        source_08a_commit_short=SOURCE_08A_COMMIT_SHORT,
        ...
        total_propagation_pool_records=len(pool),
        ...
        proposed_future_propagation=PROPOSED_FUTURE_PROPAGATION,
        ...
        anonymized_sample_ids=
            _anonymized_ids("propagation_pool_priority", len(pool)),
...
def render_markdown_summary(report: DiagnosticReport) -> str:
    lines: list[str] = []
    lines.append("# MEDAI-DOC-TYPE-UNKNOWN-DIAG-09A - Language Detector Propagation Spec")
    lines.append("")
    lines.append(f"- branch: `{report.branch}`")
    lines.append(f"- HEAD commit (short): `{report.head_commit_short}`")
    lines.append(f"- source DIAG-08A commit (short): "
                 f"`{report.source_08a_commit_short}`")
    ...
    lines.append(f"- total propagation-pool records analyzed: "
                 f"`{report.total_propagation_pool_records}`")
    lines.append(
    ...
    lines.append("## C. Proposed future propagation (NOT implemented in this block)")
    lines.append("")
    for k, v in report.proposed_future_propagation.items():
        lines.append(f"- {k}: `{v}`")
    lines.append("")
    ...
    lines.append(f"- propagation_implemented_in_this_block: "
                 f"`{report.propagation_implemented_in_this_block}`")
    ...
    lines.append("No raw filenames, raw OCR text, raw document text, private paths, "
                 ...
                 "runtime behavior change, no propagation behavior is implemented "
                 "here, no cue expansion, no OCR routing or detector behavior "
                 "change. Records remain review-bound.")
    lines.append("")
    return "\n".join(lines)
...
def write_reports(report: DiagnosticReport,
    ...
    paths = {
        "json":
            out_dir / "medai_doc_type_unknown_diag_09a_report.json",
        "md_summary":
            out_dir / "medai_doc_type_unknown_diag_09a_report.md",
        "md_main":
            out_dir / "MEDAI_DOC_TYPE_UNKNOWN_DIAG_09A.md",
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(json.dumps(
        {
            "conclusion": "medai_doc_type_unknown_diag_09a_ready",
            "files_written":
                {k: str(v.relative_to(REPO_ROOT)) for k, v in paths.items()},
            "total_propagation_pool_records": report.total_propagation_pool_records,
            ...
            "propagation_implemented_in_this_block":
                report.propagation_implemented_in_this_block,
        },
        indent=2,
    ))
    return 0
