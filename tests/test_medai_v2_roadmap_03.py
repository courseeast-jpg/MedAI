"""Focused tests for MEDAI-V2-ROADMAP-03."""
...
ALLOWED_CHANGED_PATHS = {
    "reports/medai_v2_roadmap_03/MEDAI_V2_ROADMAP_03.md",
    "reports/medai_v2_roadmap_03/medai_v2_roadmap_03_report.json",
    "reports/medai_v2_roadmap_03/medai_v2_roadmap_03_report.md",
    "scripts/run_medai_v2_roadmap_03.py",
    "tests/test_medai_v2_roadmap_03.py",
}
...
def _payload() -> dict:
    return json.loads((REPORT_DIR / "medai_v2_roadmap_03_report.json").read_text(encoding="utf-8"))
...
def test_report_summarizes_completed_v2_chain_and_registry_posture():
    payload = _payload()
    text = _report_text()
    assert len(payload["completed_v2_chain"]) == 13
    for block in (
        ...
        assert block in payload["completed_v2_chain"] or block in text
...
def test_remaining_options_ranked_and_allowed_next_posture_selected():
    payload = _payload()
    text = _report_text()
    allowed = {
        "V2-ROADMAP-PARK-02",
    ...
    assert len(payload["remaining_option_rankings"]) == 10
    assert payload["recommended_next_block"] in allowed
    assert payload["recommended_next_block"] == "V2-ROADMAP-PARK-02"
    ...
    assert "direct V2 implementation without plan/spec remain blocked" in text
...
def test_runtime_ui_launcher_db_extraction_files_not_modified_by_block():
    for path in (
        ...
        assert "MEDAI-V2-ROADMAP-03" not in path.read_text(encoding="utf-8", errors="replace")
