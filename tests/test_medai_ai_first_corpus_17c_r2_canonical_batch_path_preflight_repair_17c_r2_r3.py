"""Tests for the 17C-R2-R3 canonical batch path preflight repair. Inspect artifacts
and source only; never call a provider model."""
...
from execution.canonical_batch_paths import resolve_canonical_batch, EXPECTED_CANONICAL_BATCH
...
PUBLIC_REPORTS = ("summary.json", "implementation_report.md",
                  "canonical_batch_path_resolution_public.json", "sealed_batch_integrity_public.json",
                  "credential_preflight_public.json", "rerun_17c_r2_entry_gate.md")
# Reports that legitimately reference the mandated resolved/expected canonical path.
PATH_BEARING = {"summary.json", "implementation_report.md",
                "canonical_batch_path_resolution_public.json", "rerun_17c_r2_entry_gate.md"}
...
def test_canonical_path_resolves_and_is_a_file():
    path, exists = resolve_canonical_batch()
    assert exists is True
    assert path.is_file() is True
    assert str(path) == EXPECTED_CANONICAL_BATCH or "MedAI_Private" in str(path)


def test_resolver_robust_when_localappdata_unset():
    saved = os.environ.pop("LOCALAPPDATA", None)
    try:
        path, exists = resolve_canonical_batch()
        # The explicit literal path must still resolve (no false "missing").
        assert exists is True
        assert path.is_file() is True
    finally:
...
def test_live_loader_uses_resolver():
    src = LIVE_LOADER.read_text(encoding="utf-8")
    assert "from execution.canonical_batch_paths import resolve_canonical_batch" in src
    assert "CANON_BATCH = resolve_canonical_batch()" in src
    # The canonical batch path is no longer resolved via a bare %LOCALAPPDATA% expansion.
    assert 'CANON_BATCH = Path(os.path.expandvars(r"%LOCALAPPDATA%' not in src


def test_summary_safe_no_model_call():
    s = _summary()
    assert s["block"] == "MEDAI-AI-FIRST-CORPUS-17C-R2-CANONICAL-BATCH-PATH-PREFLIGHT-REPAIR-17C-R2-R3"
...
def test_path_and_batch_diagnostics():
    s = _summary()
    assert s["canonical_batch_exists"] is True
    assert s["canonical_batch_is_file"] is True
    assert s["canonical_batch_readable"] is True   # read-only file is readable, not missing
    assert s["canonical_batch_parent_exists"] is True
...
def test_reports_no_phi_or_secret():
    for name in PUBLIC_REPORTS:
        ...
        # Path-bearing reports legitimately reference the mandated canonical path
        # (a required diagnostic); they must still carry no PHI and no secret.
