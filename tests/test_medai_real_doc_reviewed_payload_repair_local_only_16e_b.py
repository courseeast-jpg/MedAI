"""Local-only, no-live tests for the 16E-B reviewed-payload repair.

Tests inspect only generated PUBLIC artifacts (plus existence of private/Downloads
files). They never make a provider call and never reprocess the approved input.
"""
from __future__ import annotations

import inspect
import json
import os
import re
from pathlib import Path

from clinical_knowledge.privacy import check_public_report_payload

import scripts.run_medai_real_doc_reviewed_payload_repair_local_only_16e_b as mod

REPO_ROOT = Path(__file__).resolve().parents[1]
REPORT_DIR = REPO_ROOT / "reports" / "medai_real_doc_reviewed_payload_repair_local_only_16e_b"
DOC_DIR = REPO_ROOT / "docs" / "pilot_design" / "MEDAI_REAL_DOC_REVIEWED_PAYLOAD_REPAIR_LOCAL_ONLY_16E_B"
PRIV_16E_B = Path(os.path.expandvars(r"%LOCALAPPDATA%\MedAI_Private\16E_B"))
DOWNLOADS_REVIEW = Path(os.path.expandvars(r"%USERPROFILE%\Downloads\MedAI_16E_B_Review"))

REQUIRED_DOCS = (
    "MEDAI_REAL_DOC_REVIEWED_PAYLOAD_REPAIR_LOCAL_ONLY_16E_B.md",
    "MEDAI_REPAIRED_PAYLOAD_REVIEW_RULES_16E_B.md",
    "MEDAI_OPERATOR_NO_PHI_ATTESTATION_AFTER_REPAIR_16E_B.md",
    "MEDAI_16D_RETRY_ENTRY_CRITERIA_AFTER_16E_B.md",
    "MEDAI_NEXT_DECISION_16E_B.md",
)


def _summary() -> dict:
    return json.loads((REPORT_DIR / "summary.json").read_text(encoding="utf-8"))


def test_public_reports_exist():
    for name in ("summary.json", "implementation_report.md", "repair_matrix.md",
                 "public_sanitized_repair_assessment.json"):
        assert (REPORT_DIR / name).exists(), name


def test_required_docs_exist():
    for d in REQUIRED_DOCS:
        assert (DOC_DIR / d).exists(), d


def test_private_repaired_payload_exists_outside_repo():
    assert PRIV_16E_B.is_dir()
    assert (PRIV_16E_B / "repaired_tokenized_payload.txt").exists()
    # Private dir must be outside the repository tree.
    assert REPO_ROOT not in PRIV_16E_B.parents


def test_downloads_review_copy_exists():
    assert (DOWNLOADS_REVIEW / "repaired_tokenized_payload.txt").exists()
    assert (DOWNLOADS_REVIEW / "repair_review.json").exists()
    # raw OCR and token map must NOT be copied to Downloads.
    assert not (DOWNLOADS_REVIEW / "raw_ocr.txt").exists()
    assert not (DOWNLOADS_REVIEW / "token_map_private.json").exists()


def test_summary_fields_match_required_safe_values():
    s = _summary()
    assert s["block"] == "MEDAI-REAL-DOC-REVIEWED-PAYLOAD-REPAIR-LOCAL-ONLY-16E-B"
    assert s["local_only"] is True
    assert s["one_document_only"] is True
    assert s["folder_processed"] is False
    assert s["additional_file_processed"] is False
    assert s["provider_call_made"] is False
    assert s["vertex_live_execution"] is False
    assert s["billing_api_call_made"] is False
    assert s["future_live_gate_set"] is False
    assert s["future_live_gate_environment_active"] is False
    assert s["repair_executed"] is True
    assert s["facility_identifier_tokenized"] is True
    assert s["raw_labcorp_remaining_in_repaired_payload"] is False
    assert s["clinical_terms_preservation_attempted"] is True
    assert s["false_positive_reduction_attempted"] is True
    assert s["raw_ocr_written_to_repo"] is False
    assert s["token_map_written_to_repo"] is False
    assert s["repaired_payload_written_to_repo"] is False
    assert s["public_report_phi_leak_count"] == 0
    assert s["mkb_db_opened"] is False
    assert s["active_mkb_write"] is False
    assert s["auto_accept_enabled"] is False
    assert s["medical_decision_made"] is False
    assert s["production_queue_mutated"] is False
    assert s["operator_review_required_before_live_retry"] is True
    assert s["future_16d_retry_not_started"] is True
    assert s["safety_result"] == "passed"
    assert s["privacy_result"] == "needs_human_review"


def test_script_source_has_no_provider_network_billing_imports():
    src = inspect.getsource(mod)
    for marker in (
        "import requests", "import httpx", "import aiohttp", "import urllib",
        "import google", "from google", "import vertexai", "generativeai",
        "import anthropic", "import openai", "generate_content", "cloudbilling",
        "billing_v1", "import sqlite3", "chromadb", "from clinical_knowledge.mkb",
    ):
        assert marker not in src, marker


def test_script_source_binds_approved_path_no_arg():
    src = inspect.getsource(mod)
    assert mod.APPROVED_INPUT_PATH == r"G:\MEDICAL\Urine\Archive\2024.03.22\2.PNG"
    assert "argparse" not in src
    assert "sys.argv" not in src
    assert "os.walk" not in src
    assert ".listdir" not in src
    # Gate env is only read, never set.
    assert "os.environ[" not in src


def test_public_reports_no_raw_or_payload_or_token_map():
    for name in ("summary.json", "implementation_report.md", "repair_matrix.md",
                 "public_sanitized_repair_assessment.json"):
        t = (REPORT_DIR / name).read_text(encoding="utf-8")
        assert "C:\\" not in t
        assert "/home/" not in t
        assert "Bearer " not in t
        assert "ya29." not in t
        assert "Labcorp" not in t  # raw facility identifier must not be published
        assert '"token_map"' not in t


def test_public_reports_carry_no_phi_or_secret():
    for name in ("implementation_report.md", "repair_matrix.md",
                 "public_sanitized_repair_assessment.json"):
        r = check_public_report_payload((REPORT_DIR / name).read_text(encoding="utf-8"))
        assert r.passed, (name, getattr(r, "leak_examples_redacted", None))
    rs = check_public_report_payload((REPORT_DIR / "summary.json").read_text(encoding="utf-8"))
    assert rs.raw_phi_logged_in_public_reports is False
    assert rs.secret_leaks == 0


def test_no_ssn_pattern_in_reports():
    for name in ("summary.json", "implementation_report.md", "repair_matrix.md",
                 "public_sanitized_repair_assessment.json"):
        t = (REPORT_DIR / name).read_text(encoding="utf-8")
        assert not re.search(r"\b\d{3}[- ]\d{2}[- ]\d{4}\b", t)
