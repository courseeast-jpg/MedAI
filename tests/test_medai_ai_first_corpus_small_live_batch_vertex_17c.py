"""No-live tests for the 17C small live batch. Inspect generated artifacts only;
...
PUBLIC_REPORTS = (
    "summary.json", "implementation_report.md", "live_batch_status_public.json",
    "schema_validation_summary_public.json", "cost_guard_public.json", "privacy_gate_matrix.md",
...
def test_no_raw_response_or_payload_body_in_repo():
    for name in PUBLIC_REPORTS:
        t = (REPORT_DIR / name).read_text(encoding="utf-8")
        assert '"response"' not in t           # raw provider response object never published
        assert '"tokenized_content"' not in t  # request payload body never published
...
def test_summary_required_safety_fields():
    ...
    assert s["future_live_gate_environment_active_after_run"] is False
    ...
    assert s["hard_cost_cap_usd"] == 0.05
    assert s["estimated_cost_before_run_usd"] <= 0.05
...
def test_execution_result_branch_consistency():
    ...
    if result == "PASS":
        assert s["request_count_succeeded"] == 12
        assert s["request_count_failed"] == 0
        assert s["stopped_on_first_failure"] is False
    else:
        # Non-pass outcomes must be a clean stop with consistent counts.
        assert s["stopped_on_first_failure"] is True
        assert s["request_count_succeeded"] <= s["request_count_sent"] <= s["request_count_loaded"]
        assert s["request_count_succeeded"] + s["request_count_failed"] <= s["request_count_sent"] or s["request_count_sent"] == 0
        assert s["failure_stage"] in ("preflight", "provider", "parse", "schema", "privacy", "cost", "gate")
...
def test_public_reports_carry_no_phi_or_secret():
    # Non-summary reports fully pass; summary may reference the two mandated private
    # paths (required fields) but must carry no PHI/secret.
    for name in PUBLIC_REPORTS:
