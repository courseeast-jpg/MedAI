"""Focused tests for MEDAI-DOC-TYPE-UNKNOWN-DIAG-15B.

Verifies the aggregate-only layout/table extraction audit:

* targets only the 10 Sub-track B records (Sub-track A PDF-text audit
...
from scripts.run_medai_doc_type_unknown_diag_15b_layout_table_extraction_audit import (
...
def test_targets_only_10_sub_track_b_records():
    a = audit_sub_track_b(_synthetic_diag14_payload())
    assert a["total_records_analyzed"] == 10


def test_layout_table_buckets_present_and_named():
    a = audit_sub_track_b(_synthetic_diag14_payload())
    assert set(a["layout_table_extraction_bucket_counts"].keys()) == set(
        LAYOUT_TABLE_BUCKETS
    )


def test_layout_table_definitional_buckets():
    a = audit_sub_track_b(_synthetic_diag14_payload())
    c = a["layout_table_extraction_bucket_counts"]
    assert c["table_structure_visible_but_text_insufficient"] == 10
    assert c["layout_structure_visible_but_family_classifier_input_sparse"] == 10
    assert c["table_header_or_label_context_insufficient"] == 10
    assert c["numeric_or_grid_like_content_without_family_cues"] == 10
    assert c["possible_pdf_table_extraction_gap"] == 10
    assert c["metadata_sufficient_for_future_layout_audit"] == 10


def test_layout_table_negative_buckets_are_zero():
    a = audit_sub_track_b(_synthetic_diag14_payload())
    c = a["layout_table_extraction_bucket_counts"]
    assert c["row_or_column_structure_likely_lost"] == 0
    assert c["possible_multi_column_or_fragmented_text_order_issue"] == 0
    assert c["insufficient_safe_metadata"] == 0


def test_anonymous_ids_are_file_nnn_only_and_count_is_10():
    a = audit_sub_track_b(_synthetic_diag14_payload())
    ids = a["anonymous_ids_used"]
    assert len(ids) == 10
    assert ids[0] == "file_001"
    assert ids[-1] == "file_010"
    for ident in ids:
...
def test_exclusion_audit_excludes_sub_track_a_and_other_pools():
    e = excludes_other_pools_audit()
    for key in [
        "pdf_text_extraction_quality_audit_pool_excluded",
        "non_text_layer_pools_excluded_from_diag15b_buckets",
...
def test_build_report_identity():
    p = build_report()
    assert p["phase_id"] == PHASE_ID == "MEDAI-DOC-TYPE-UNKNOWN-DIAG-15B"
    assert p["mode"] == "evaluation_only"
    assert p["aggregate_only"] is True
    assert p["subtrack"] == SUBTRACK_ID == "B_layout_table_extraction_audit"
...
def test_build_report_total_records_analyzed():
    p = build_report()
    assert p["total_records_analyzed"] == 10


def test_build_report_required_safety_flags_are_false():
    p = build_report()
    for flag in [
        "behavior_changed",
        "external_api_used",
...
def test_build_report_progress_percentages_present():
    ...
    assert before["residual_unknown_reduction_track_done_pct"] == 99.5
    assert before["residual_unknown_reduction_track_remaining_pct"] == 0.5
    assert before["whole_medai_project_done_pct"] == 89
    assert before["whole_medai_project_remaining_pct"] == 11
    assert after["residual_unknown_reduction_track_done_pct"] == 99.8
    assert after["residual_unknown_reduction_track_remaining_pct"] == 0.2
    assert after["whole_medai_project_done_pct"] == 89.5
    assert after["whole_medai_project_remaining_pct"] == 10.5
...
def test_build_report_lists_four_source_blocks_including_diag_15():
    p = build_report()
...
def test_rendered_markdown_mentions_all_buckets():
    ...
    assert "Layout/table extraction bucket counts" in md
...
def test_rendered_short_summary_includes_progress():
    ...
    assert "99.8%" in short and "0.2%" in short
...
def test_written_json_records_correct_counts(written_files):
    payload = json.loads(written_files["json"].read_text(encoding="utf-8"))
    assert payload["total_records_analyzed"] == 10
...
# ── ensure DIAG-15B added no runtime helper or UI surface ─────────────────


def test_no_diag_15b_runtime_helper_imported_into_app():
    ...
    assert "run_medai_doc_type_unknown_diag_15b" not in body
    assert "render_plan_for_layout_table_extraction" not in body


def test_no_clinical_knowledge_helper_for_diag_15b():
    ck_dir = Path(__file__).resolve().parents[1] / "clinical_knowledge"
    for path in ck_dir.rglob("*.py"):
        body = path.read_text(encoding="utf-8", errors="ignore")
        assert "MEDAI-DOC-TYPE-UNKNOWN-DIAG-15B" not in body, (
            f"DIAG-15B added a runtime helper at {path}, but it must remain "
            "evaluation-only."
        )
