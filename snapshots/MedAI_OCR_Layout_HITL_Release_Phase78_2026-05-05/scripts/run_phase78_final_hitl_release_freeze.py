...
def _rmtree_safe(path: Path) -> None:
    """Delete directory tree, using extended UNC path on Windows to handle long paths."""
    if sys.platform == "win32":
        unc = "\\\\?\\" + str(path.resolve())
...
def _copy_snapshot(src_root: Path, dest_root: Path) -> list[str]:
    """Copy git-tracked safe files to dest_root; returns list of included paths."""
    # Use git ls-files for speed and accuracy — only tracked files
    result = subprocess.run(
        ["git", "ls-files"],
        cwd=src_root,
        capture_output=True,
