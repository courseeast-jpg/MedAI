...
def test_phase28_outputs_are_written(tmp_path: Path):
    ...
    assert metrics["pre_run_review_queue_items"] == 31
    assert metrics["current_run_review_queue_items"] == 31
    ...
    assert "Pre-run review queue items (gate input)" in report_text
    assert "Current run review queue items" in report_text
